...
# 依需求排除「預估空屋率=0」的房源:本頁範圍(房東下拉間數、五個分頁的統計與
# 卡片)一律不計入 vac_pred 剛好為 0 的列。僅影響本頁,不動其他頁面的預測資料。
PREDS = PREDS[PREDS["vac_pred"] != 0].reset_index(drop=True)
BUNDLE = get_bundle()
...
with st.sidebar:
    ...
    def _on_host_change():
        """切換房東時立即清除行政區／房型及房源下拉的 session state，
        確保下一次 rerun 時 multiselect 使用新房東的 default（全選）。"""
        # 取得新選中的 host_id
        _new_idx = st.session_state.get("host_selectbox", 0)
        _new_host_id = int(_hc.iloc[_new_idx]["host_id"])
        st.session_state["_active_host_id"] = _new_host_id
        for _k in ("card_district", "card_room", "nb_sel", "risk_sel", "cal_sel", "rep_sel"):
            st.session_state.pop(_k, None)

    _hi = st.selectbox("host", range(len(_hc)), format_func=lambda i: _hlab[i],
                       label_visibility="collapsed", key="host_selectbox",
                       on_change=_on_host_change)
    host_id = int(_hc.iloc[_hi]["host_id"])
    MY = PREDS[PREDS["host_id"] == host_id].reset_index(drop=True)

    # 備援：若初次載入或其他情況下 _active_host_id 未同步，補齊清除動作。
    if st.session_state.get("_active_host_id") != host_id:
        st.session_state["_active_host_id"] = host_id
        for _k in ("card_district", "card_room", "nb_sel", "risk_sel", "cal_sel", "rep_sel"):
            st.session_state.pop(_k, None)
    ...
    st.divider()
    ui_kit.model_spec(explain=False)     # D2:三處共用同一份技術口徑(側欄不再列 LIME)
...
TBREG, TB1, TB2P, TB5, TB2, TB6 = st.tabs(
    ["📝 房源登錄", "🗂️ 房源總表", "💰 房源定價情報", "🗓️ 未來檔期",
     "⚠️ 風險診斷", "🧾 月報"])
TB3 = TB2P          # 「周邊比較」內容併入定價情報分頁

# ── 房源登錄(刊登前風險試算;放在房源總表左邊)──
with TBREG:
    from modules import listing_registration as LR
    LR.render(BUNDLE)
...
def trend_arrow(row):
    """顯示『模型預估空房率』與所屬行政區中位數的差距(與環上的預估數字一致)。"""
...
with TB1:
    ...
    _overview = [
        ...
        ("30 天滿房比例", "—" if SCOPE["booked_rate_d30"].isna().all()
    ...
    for _col, (_label, _value) in zip((k1, k2, k3, k4), _overview):
        with _col:
            if _label == "需優先處理":
    ...
    for _s in range(0, len(_cards), 3):
        ...
        for _c, (_, r) in zip(cols, _cards.iloc[_s:_s + 3].iterrows()):
            ...
            # 環的『數字』= 模型預估空房率(vac_pred,專案核心產出);
            # 環的『顏色』= 象限風險色(綜合模型+真實檔期),不再用預估值本身上色。
            # 這樣隱形危機/真警報的卡會是琥珀/紅環,不會用綠色假裝安全;
            # 環下另秀『真實 90 天空房率』當對照,讓「預估 vs 實際」的落差一目了然。
            _booked90 = pd.to_numeric(r.get("booked_rate_d90"), errors="coerce")
            _real_vac_txt = "—" if pd.isna(_booked90) else f"{(1 - float(_booked90)):.0%}"
            _ring_val = float(r["vac_pred"])
            _ring_c = P[QD.QUADRANTS.get(r.get("quadrant"), {}).get("color", "muted")]
            with _c:
...
<div class="listing-card-meta">
  ...
  <div class="listing-card-meta-row"><span class="listing-card-meta-key">房型</span>
    <span class="listing-card-meta-value">{_room} · {_capacity_txt}</span></div>
                    ...
                    with _risk_col:
                        st.markdown(f"""
<div class="listing-card-risk" aria-label="預估空房率與實際訂房狀態">
  <div class="listing-card-risk-label">預估空房率</div>
  <div class="listing-card-ring">{risk_ring(_ring_val, _ring_c, size=122)}</div>
  {trend_arrow(r)}
  <div class="listing-card-calendar">90 天實際空房率<strong>{_real_vac_txt}</strong></div>
</div>""", unsafe_allow_html=True)
...
with TB2P:
    ...
    with st.container(key="pricing-controls"):
        ...
        with _c_sel:
            _sel_label, _sel_input = st.columns([.18, 1],
                                                vertical_alignment="center")
            _sel_label.markdown(
                "<div class='pricing-control-label'>選擇房源</div>",
                unsafe_allow_html=True)
            with _sel_input:
                _sel3 = st.selectbox("選擇房源", list(_opt_lab.keys()), key="nb_sel",
                                     label_visibility="collapsed")
        with _c_rad:
            with st.container(key="pricing-radius-control"):
                _rad_label, _rad_input = st.columns([.5, 1],
                                                    vertical_alignment="center")
                _rad_label.markdown(
                    "<div class='pricing-control-label'>📏 附近比較半徑 (公尺)</div>",
                    unsafe_allow_html=True)
                with _rad_input:
                    radius = st.slider("附近比較半徑 (公尺)", 500, 2000, 1000, step=100,
                                       key="cmp_radius", label_visibility="collapsed")
    ui_kit.section_header("同業比價")
bid = _opt_lab[_sel3]
...
with TB2:
    _sel = st.selectbox("選擇房源", list(_opt_lab.keys()), key="risk_sel")
...
<div class="pricing-anchor">
  ...
  <span class="pricing-anchor-sub">每晚 ${float(B['price']):,.0f} ·
    ...
    _rank = (int((NB[PROB_COL] < float(B[PROB_COL])).sum()) + 1) \
        if len(NB) else 1
    _pricing_kpis = [
        (f"{radius}m 內 Airbnb", f"{len(NB_o)} 間", None),
        ("周邊高風險機率中位",
         f"{NB_o[PROB_COL].median()*100:.0f}%" if len(NB_o) else "—", None),
        ("我的高風險機率 vs 周邊", f"{B[PROB_COL]*100:.0f}%",
         f"↑ 高於 {_pctl_rk:.0%} 的鄰居" if _pctl_rk is not None else None),
        ("同商圈排名(低風險優先)", f"第 {_rank} / {len(NB)} 名", None),
    ]
    for _kpi_col, (_kpi_label, _kpi_value, _kpi_note) in zip(
            st.columns(4, gap="small"), _pricing_kpis):
        with _kpi_col:
            overview_metric_card(_kpi_label, _kpi_value, _kpi_note)

    ui_kit.section_header("周邊房源分布圖")
