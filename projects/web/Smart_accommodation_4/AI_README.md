...
- `tests/`：186 案，涵蓋通知組合、平台分析邏輯、租客五科計分、租客 NLP 正面優點、風險管理 AppTest，以及設計系統的反迴歸守門（`test_design_tokens.py`／`test_ui_kit.py`／`test_ui_consistency.py`）。

## 📌 當前開發進度（最後更新：2026-07-24）

### 2026-07-24 租客入口 NLP「住客常提到的優點」

- [x] 詳細分析右側圖表由未清理原始詞頻改為 `tenant-positive-aspects-v1.0` 雙語優點主題；統計單位是「提及該優點的有效正面評論則數」，同則同主題與重複評論不重複計數。
- [x] 情感分佈與優點圖共用最近最多 20 則評論；英文用 VADER、中文用 jieba＋中文詞典、中英混合合併兩者訊號，空白、過短、不支援語言與分析器缺失均明確列為無法分析。
- [x] 完成 0、1～2、3～4、5+ 正面評論顯示狀態，並顯示取樣數、有效／無法分析數、涵蓋率、有效正面數、實際分析器與規則版本；未修改 N、R、S、Q 或推薦排序公式。
- [x] 實際房源 `14582309` 驗收：269 則總評論中取最近 20 則，19 則有效且為正面；輸出地點便利 4、居住舒適 4、環境乾淨 2、鄰近捷運 1、房東友善 1、空間寬敞 1，未再出現 `and`、`the`、`to`、`is`、`very`、`was`。
- [x] 租客 NLP、計分隔離、舊摘要相容性與 UI 規範測試 79 passed；租客入口 AppTest 0 exceptions、四個詳情分頁與 5 張 Plotly 圖正常渲染。
- [/] 完整回歸 185 passed、1 failed；唯一失敗是未修改的後台分析載入既有模型時找不到舊 scikit-learn `_loss` 模組，屬本機 Python 3.14／模型序列化版本相容性，需在後台模型任務另行處理。
...
- 2026-07-24（租客 NLP 正面優點）：`modules/nlp_analysis.py` 新增共用最近評論分析與租客專用正面優點摘要。優點主題第一版固定為環境乾淨、地點便利、鄰近捷運、居住舒適、房東友善、環境安靜、空間寬敞、價格划算；`great`／`nice`／`good` 不可單獨形成優點。規則版本為 `tenant-positive-aspects-v1.0`，日後調整詞表、同義詞或門檻時須升版並補測試。房東端 `listing_review_summary()` 與全域後台統計保留既有情緒分析路徑，避免租客規則改變其他入口。部署依賴新增 `vaderSentiment>=3.3.2` 與 `jieba>=0.42.1`；本機 Python 3.14 無法編譯專案既定的 `shap~=0.49.1`，因此本次只安裝租客入口驗收所需套件，未修改 SHAP 版本。
