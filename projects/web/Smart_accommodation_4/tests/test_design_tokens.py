...
# ── 房源卡風險環:依空屋率上色 ──────────────────────────────────
@pytest.mark.parametrize("vac, expected", [
    (0.60, "red"),      # 含下界:剛好 60% → 紅
    (1.00, "red"),
    (0.59, "yellow"),
    (0.35, "yellow"),   # 含下界:剛好 35% → 黃
    (0.34, "green"),
    (0.00, "green"),
])
def test_vac_ring_tier_thresholds(vac, expected):
    """環色門檻含下界,與 RISK_TIERS 文案「≥60% / 35–60% / <35%」對齊。"""
    assert dt.vac_ring_tier(vac) == expected


@pytest.mark.parametrize("bad", [float("nan"), None, "x", ""])
def test_vac_ring_tier_missing_returns_none(bad):
    """非有限值回 None,讓呼叫端自行決定退回策略;不得丟例外。"""
    assert dt.vac_ring_tier(bad) is None


def test_vac_ring_tier_reuses_semantic_colors():
    """環色一律經 tier_color 取既有語意色,不引入新色碼。"""
    assert dt.tier_color(dt.vac_ring_tier(0.80)) == dt.COLOR["danger"]
    assert dt.tier_color(dt.vac_ring_tier(0.50)) == dt.COLOR["warning"]
    assert dt.tier_color(dt.vac_ring_tier(0.10)) == dt.COLOR["success"]
