...
def _fmt_money(v) -> str:
    try:
        ...
        return "—"
    if f != f or f <= 0:        # NaN 或無價格資料
        return "—(無價格資料)"
    return f"NT$ {f:,.0f}"


def _safe_name(v, lid) -> str:
    """房源名稱缺值兜底(NaN 為 truthy,不能用 `or`)。"""
    if v is None or (isinstance(v, float) and v != v):
        return f"房源 #{lid}"
    s = str(v).strip()
    return s if s and s.lower() != "nan" else f"房源 #{lid}"


def _safe_price(v) -> float:
    """價格缺值回傳 0.0(呼叫端以 _fmt_money 顯示為「—」)。"""
...
def collect(listing_row, pred_row, listings_df,
    ...
    d = {
        "listing_id": lid,
        # NaN 是 truthy,`x or fallback` 會留下 NaN → 印出 "nan"/"$nan"
        "name": _safe_name(pred_row.get("name"), lid),
        "district": pred_row.get("neighbourhood_cleansed", ""),
        "room_type": pred_row.get("room_type", ""),
        "price": _safe_price(pred_row.get("price")),
    ...
    if cal is not None:
        ...
        q = classify_row(d["tier"], d["cal"]["d90"], d["vac_pred"])
        d["quadrant"] = q
...
def to_markdown(d: dict, summary_src: str, summary: str) -> str:
    ...
    if c:
        ...
        if m is not None and len(m):
            L += ["### 月度訂房率 vs 同商圈", "",
                  "| 月份 | 本房源 | 同商圈基準 | 差距 | 可否判讀 |",
                  "|---|---|---|---|---|"]
            for _, r in m.iterrows():
                ok = bool(r.get("可判讀", True))
                base = ("—" if pd.isna(r["同商圈基準"])
                        else f"{r['同商圈基準']:.0%}")
                diff = "—" if pd.isna(r["差距"]) else f"{r['差距']*100:+.0f} pp"
                mark = "✅" if ok else f"⚪ {r.get('不可判讀原因', '')}"
                L.append(f"| {r['月份']} | {r['本房源']:.0%} | {base} | "
                         f"{diff if ok else '—'} | {mark} |")
            L += ["", "> ⚪ 標記的月份不列入判讀:Inside Airbnb 的「不可訂」同時包含"
                      "已被預訂與房東主動封鎖,遠期月份多為日曆尚未開放,"
                      "與同商圈平均比較沒有意義。", ""]
