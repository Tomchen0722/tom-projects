...
class SuggestionEngine:
    ...
    def suggest(self, shap_items: list, comp_stats: dict, features: dict,
                own_amenities: set, method: str = "LIME") -> list:
        """產出建議清單(依優先度排序)。

        shap_items: [(feature, phi)],已依 phi 由大到小排序、僅含 phi>0。
        comp_stats: CompetitorIndex.stats() 輸出。
        features: 該房源目前特徵值 dict。
        method:   歸因方法名稱,用於「依據」說明。房東入口傳入的是
                  **LIME 局部線性近似**的權重;後台 SHAP 分頁才是 TreeExplainer
                  的 SHAP 值。此參數避免把 LIME 結果誤標為 SHAP(2026-07-22 修正)。
        """
        cfg = self.config
        out = []
        ...
        for feat, phi in shap_items[: cfg["shap_top_k"]]:
            ...
            out.append({
                "type": "attribution", "title": title, "detail": detail,
                "evidence": f"此因素推高高風險機率 {phi * 100:+.1f} 個百分點"
                            f"({method} 局部歸因)",
                "priority": 1,
            })
