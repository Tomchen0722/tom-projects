...
def vac_ring_tier(vac) -> str | None:
    """把空屋率(0–1 小數)映成 red/yellow/green 的 tier key;非有限值回 None。

    給房源卡風險環用:環上顯示的數字是預估空屋率,顏色也直接反映同一個數值,
    讓數字與顏色一致(不同於全站 tier 走 prob=P(空屋率≥60%))。
    門檻含下界,與 RISK_TIERS 文案「≥60% / 35–60% / <35%」對齊:
    >=0.60 紅、>=0.35 黃、<0.35 綠。顏色一律再經 tier_color() 取,不新增色碼。
    """
    try:
        v = float(vac)
    except (TypeError, ValueError):
