...
# ─── PoI Loaders ────────────────────────────────────────────────
def _load_mrt():
    """Load MRT station exits with coordinates(改讀清洗版 mrt_clean.csv)。"""
    path = DATA_DIR / "mrt_clean.csv"
    try:
...
def _load_restaurants():
    ...
    path = DATA_DIR / "restaurants_clean.csv"
    try:
        df = pd.read_csv(path, encoding="utf-8-sig")
    except Exception:
        df = pd.read_csv(path, encoding="big5")
    cols = df.columns.tolist()
    # Columns: 統一編號, 商業名稱, 商業地址, Longitude, Latitude
...
def _load_schools():
    ...
    path = DATA_DIR / "school_clean.csv"
    try:
        df = pd.read_csv(path, encoding="utf-8-sig")
    except Exception:
        df = pd.read_csv(path, encoding="big5")
    cols = df.columns.tolist()
    # Columns: 項次, school, schoolname, postalcode, address, telephone, 經度, 緯度
...
def _load_clinics():
    """Load clinic PoI data(改讀清洗版 clinics_clean.csv;原為 JSON)。"""
    path = DATA_DIR / "clinics_clean.csv"
    df = pd.read_csv(path, encoding="utf-8-sig")
    df.columns = [c.lstrip("﻿") for c in df.columns]
    # Columns: lat, lon, name, address
    df = df.rename(columns={"lon": "longitude", "lat": "latitude"})
    return df[["name", "address", "latitude", "longitude"]].dropna(subset=["latitude", "longitude"])


def _load_parks():
    """Load park PoI data(改讀清洗版 parks_clean.csv;原為 JSON)。"""
    path = DATA_DIR / "parks_clean.csv"
    df = pd.read_csv(path, encoding="utf-8-sig")
    df.columns = [c.lstrip("﻿") for c in df.columns]
...
def _load_convenience():
    ...
    path = DATA_DIR / "conbini_clean.csv"
    try:
        df = pd.read_csv(path, encoding="utf-8-sig")
    except Exception:
