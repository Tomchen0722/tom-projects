...
# ─── UI helper functions ────────────────────────────────────────
def money(v, prefix="$", suffix="", dash="價格未提供"):
    """安全的金額格式化(2026-07-22 修正)。

    為什麼需要:全量預測表 5,849 筆中有 701 筆(12%)沒有價格,
    直接用 f"${v:,.0f}" 會印出「$nan」給房東看。缺值一律顯示 dash。
    """
    try:
        f = float(v)
    except (TypeError, ValueError):
        # 相容 "$1,238.43" 這類含貨幣符號與千分位的字串
        import re as _re
        s = _re.sub(r"[^\d.\-]", "", str(v))
        try:
            f = float(s)
...
def safe_text(v, fallback="(未命名房源)", limit=None):
    """安全的文字取值(2026-07-22 修正)。

    為什麼需要:Python 的 float('nan') 是 truthy,
    `str(x or fallback)` 遇到缺值會回傳字串 "nan" 而非 fallback。
    """
    if v is None:
        return fallback
