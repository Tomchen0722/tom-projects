...
def review_sentiment_breakdown(reviews_df, listing_id, window=20):
    ...
    """
    from modules.nlp_analysis import analyze_recent_reviews
    analysis = analyze_recent_reviews(reviews_df, listing_id, window=window)
    n_total = analysis["total_reviews"]
    if n_total == 0:
        return {"pos_n": 0, "neg_n": 0, "neu_n": 0,
                "n_analyzable": 0, "n_total": 0}
    rows = analysis["rows"]
    valid = rows[rows["valid"].fillna(False).astype(bool)]
    pos = int((valid["label"] == "正面").sum())
    neg = int((valid["label"] == "負面").sum())
    neu = int((valid["label"] == "中立").sum())
    analyzable = analysis["analyzable_count"]
