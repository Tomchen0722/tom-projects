...
def _caveat(body):
    """但書提示條(黃底)。用於「這個數字不能拿來做什麼」的限制聲明。"""
    return (
        f"<div style='background:var(--sa-warning-bg);"
        f"border:1px solid var(--sa-warning-border);"
        f"border-radius:var(--sa-radius-sm);padding:10px 16px;"
        f"margin:var(--sa-space-3) 0 2px;font-size:var(--sa-text-body);"
        f"color:var(--sa-warning-fg);line-height:1.5;'>{body}</div>")


@st.cache_data(show_spinner=False)
def _actual_compare(commission: float):
    """⑤ 用的預估 vs 實際對照;缺任一產物回傳 None(呼叫端顯示提示)。"""
    preds = load_predictions()
    if preds is None or not aa.available():
        return None
    cols = ["id", "host_id", "price", "vac_pred_365"]
    actual = aa.load_actual()
    return (aa.compare_kpis(preds[cols], actual, commission),
            aa.population_note(preds[cols], actual))


def _money_short(v: float) -> str:
    """金額縮寫:億 / 萬(與 platform_sections._money 同口徑)。"""
    if abs(v) >= 1e8:
        return f"${v / 1e8:,.2f} 億"
    if abs(v) >= 1e4:
        return f"${v / 1e4:,.1f} 萬"
...
def _fmt_gap(v, kind: str) -> str:
    """落差一律帶正負號;比率用百分點(pp)而非百分比,避免與比率本身混淆。"""
    if kind == "count":
        return f"{int(v):+,} 位"
    if kind == "pct":
        return f"{v * 100:+.1f}pp"
    return ("+" if v >= 0 else "-") + _money_short(abs(v))
...
def render_data_analysis():
    ...
    _render_actual_vs_pred()


# ════════════════════════════════════════════════════════════════
# ⑤ 預估 vs 實際 —— 拿真實已發生資料回頭對照
# ════════════════════════════════════════════════════════════════
def _render_actual_vs_pred():
    """⑤ 區塊:五項 KPI 的預估 vs 實際對照 + 成因 + 方法論但書。

    刻意放在①~④之後:①②講模型的正式評估(GroupKFold OOF、前瞻驗證),
    本區塊是「拿另一份真實資料回頭看」的補充視角,不是準確度評估
    (成因無法拆開,見 aa.CAVEAT)。
    """
    ui_kit.section_header(
        "預估 vs 實際：拿真實已發生資料回頭對照",
        desc=f"用 {aa.WINDOW_LABEL} 期間的真實評論資料回推實際入住，"
             f"與當初的模型預估值並列")

    res = _actual_compare(0.15)
    if res is None:
        ui_kit.empty_state(
            "尚未產出實際入住指標",
            hint="缺少 data/_actual_metrics.csv，請先執行下列腳本產出。",
            cmd="python -X utf8 scripts/build_actual_metrics.py",
            icon="⚙️")
        return
    cmp_df, pop = res
    ...
    show = pd.DataFrame({
        "指標": cmp_df["指標"],
        "當初預估": [_fmt_value(v, k) for v, k in zip(cmp_df["預估"],
                                                      cmp_df["格式"])],
        "實際發生": [_fmt_value(v, k) for v, k in zip(cmp_df["實際"],
                                                      cmp_df["格式"])],
        "落差": [_fmt_gap(v, k) for v, k in zip(cmp_df["落差"],
                                                cmp_df["格式"])],
    })
    c1, c2 = st.columns([1.25, 1])
    with c1:
        ui_kit.data_table(show, height=250)
        st.markdown(_callout(
            f"最該注意的一行：平台年收入當初預估 "
            f"{_money_short(float(cmp_df.iloc[4]['預估']))}，"
            f"按實際入住推算為 "
            f"{_money_short(float(cmp_df.iloc[4]['實際']))}"
            f"（{_fmt_gap(float(cmp_df.iloc[4]['落差']), 'money')}）。"),
            unsafe_allow_html=True)
    with c2:
        roles = ["danger", "warning", "muted"]
        cards = [_insight_card(t, b, roles[i])
                 for i, (t, b) in enumerate(aa.CAUSES)]
        st.markdown("<div style='display:grid;gap:8px;'>" + "".join(cards)
                    + "</div>", unsafe_allow_html=True)

    st.markdown(_caveat(aa.CAVEAT), unsafe_allow_html=True)
    # note() 走 HTML div,Streamlit 不處理其中的 markdown → 強調用 <strong>
    note(
        f"<strong>計算口徑</strong>："
        f"實際入住採 Inside Airbnb 官方公式（窗口內評論數 × 2 × "
        f"max(最短入住, 3)，含上限），已在本專案資料上 100% 精確重現；"
        f"× 2 是官方假設僅半數住客會留評論。<br>"
        f"<strong>窗口</strong>：{aa.WINDOW_LABEL}（共 {aa.WINDOW_DAYS} 天）。"
        f"模型預估為 365 天，兩者長度不同；缺少的 7–9 月經歷史評論分布檢驗，"
        f"季節性影響約 1 個百分點，故未做校正。<br>"
        f"<strong>母體</strong>：全平台 {pop['total']:,} 間中，"
        f"排除 {pop['dormant']:,} 間"
        f"「前後兩期都零評論」的無經營跡象房源（非衰退，是本來就沒在營業），"
        f"納入 {pop['kept']:,} 間。預估欄與實際欄採同一批房源才公平，"
        f"故此處的預估值<strong>不等於</strong>市場總覽卡片的全母體數字。")
