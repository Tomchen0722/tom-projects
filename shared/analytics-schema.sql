...
-- 【已修正】明確指定時區，使表達式符合 IMMUTABLE 要求
create index if not exists idx_pv_daily on public.page_views (project_id, ((viewed_at at time zone 'Asia/Taipei')::date));
...
-- 任何人都可以新增一筆瀏覽紀錄（這是計數器的本質）
drop policy if exists "anyone can insert view" on public.page_views;
create policy "anyone can insert view" on public.page_views 
...
-- ── 3. 彙總 View（供前端讀取，不暴露原始資料）───────────────
-- 3-1 每個專案的每日瀏覽數
create or replace view public.v_views_daily with (security_invoker = off) as
select 
    project_id,
    (viewed_at at time zone 'Asia/Taipei')::date as day,
    count(*) as views,
...
-- 3-2 每個專案的每月瀏覽數
create or replace view public.v_views_monthly with (security_invoker = off) as
select 
    project_id,
    to_char(viewed_at at time zone 'Asia/Taipei', 'YYYY-MM') as month,
    count(*) as views,
...
-- 3-3 每個專案的總計 + 今日 + 本月（前端主要讀這張）
create or replace view public.v_views_summary with (security_invoker = off) as
select 
    project_id,
    count(*) as total_views,
    count(distinct visitor_id) as total_visitors,
    count(*) filter (
        where (viewed_at at time zone 'Asia/Taipei')::date = (now() at time zone 'Asia/Taipei')::date
    ) as today_views,
    count(*) filter (
        where to_char(viewed_at at time zone 'Asia/Taipei', 'YYYY-MM') = to_char(now() at time zone 'Asia/Taipei', 'YYYY-MM')
    ) as month_views,
    max(viewed_at) as last_viewed_at
from public.page_views
group by project_id;
...
-- 3-4 專案內各頁面的熱門度（看哪一課最多人讀）
create or replace view public.v_views_by_page with (security_invoker = off) as
select 
    project_id,
    coalesce(page_path, '(index)') as page_path,
    count(*) as views
